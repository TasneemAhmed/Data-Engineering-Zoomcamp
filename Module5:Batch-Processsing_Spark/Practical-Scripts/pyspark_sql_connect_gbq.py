...
def main(params):
    ...
    bucket = "dataproc-temp-us-central1-499748336022-9gtvdghy"
    ...
    zones_df_join = zones_df_join.drop('zone', 'LocationID')
    # Save the data to BigQuery
    zones_df_join.write.format('bigquery') \
      .option('table', joined_output) \
        .save()
...
Use Dataproc to submit the PySpark code: Instead of running the PySpark code manually from the cluster master instance, 
you can submit the PySpark file directly to your cluster. 
...
gcloud dataproc jobs submit pyspark pyspark_sql_connect_gbq.py --cluster=spark-cluster --region=us-central1 --jars=gs://spark-lib/bigquery/spark-bigquery-latest.jar
--project=module5-spark -- --green_path=gs://spark-output-bi-reports/green_dataset/green_tripdata_2020-01.csv --yellow_path=gs://spark-output-bi-reports/yellow_dataset/yellow_tripdata_2020-01.csv --zones_path=gs://spark-output-bi-reports/taxi+_zone_lookup.csv
--output_path=module5-spark.taxi_report_2020.january_report
"""